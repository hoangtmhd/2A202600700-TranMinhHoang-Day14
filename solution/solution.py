...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            is_rel = overlap_ratio >= relevance_threshold
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            f"Grade the answer based on the following rubric:\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_desc}\n"
            f"Please output a JSON object containing key-value pairs where the keys are the criteria names and values are float scores in [0.0, 1.0]."
        )
        try:
        ...
        import json
        try:
            scores = json.loads(cleaned_str)
            final_scores = {}
            for k in rubric.keys():
                if k in scores:
                    try:
                        final_scores[k] = float(scores[k])
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(all_scores) >= 2:
            ...
            avg_first = sum(all_scores[:mid]) / mid
            avg_second = sum(all_scores[mid:]) / (len(all_scores) - mid)
            if avg_first - avg_second > 0.15:
                positional_bias = True

        avg_batch_score = sum(all_scores) / len(all_scores) if all_scores else 0.5
        leniency_bias = avg_batch_score > 0.8
        severity_bias = avg_batch_score < 0.3 if all_scores else False
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        for f in failures:
            if f.failure_type:
                cats[f.failure_type] = cats.get(f.failure_type, 0) + 1
        return cats
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_score = min(score for name, score in scores)
        lowest_metrics = [name for name, score in scores if score == min_score]
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            if i < len(suggestions):
                ...
                fix = suggestions[i % len(suggestions)]
            else:
